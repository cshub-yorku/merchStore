...
export default function CSCard({ productState, data, src, onClick }) {
  ...
  return (
    <div className="flex flex-col bg-[#624F82] rounded-md">
      <Box
        component="img"
        onClick={() => {
          onClick();
      ...
      <Box display="flex" alignItems="center" sx={cardNav(theme)}>
        <Box sx={cardTextBox}>
          <Typography variant="h5" sx={[cardLabel, bold, fontInter]}>
            {name}
          </Typography>
          <Typography variant="h6" sx={[cardSubtitle, normal, fontInter]}>
            Type
          </Typography>
        </Box>
        <Button
